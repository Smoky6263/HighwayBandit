...
public class PlayerAccessoryWearLogic : MonoBehaviour
{
    [SerializeField] private SkinnedMeshRenderer m_characterRenderer;

    [SerializeField] private PlayerAccessoryLogic[] m_accessoriesToAttach = null;
    ...
    private void Awake()
    {
        ...
        foreach (PlayerAccessoryLogic a in m_accessoriesToAttach) { Attach(a); }
    }

    public void Attach(PlayerAccessoryLogic accessory)
    {
        if (!m_initialized)
        {
            Initialize(gameObject);
